...
def decode(ba):
    ...
    while True:
        ...
        opc = opcodes[b]
        if opc['params'] == 0:
            out.append([i, opc['name']])
        elif opc['params'] > 0:
            att = [next(bs)[1] for _ in range(opc['params'])]
            out.append([i, opc['name'], att])
        elif opc['params'] < 0:
            if b == 170:
                ...
                out.append([i, opc['name'], padd + [default, low, high] + offsets])
            elif b == 171:
                padd = [next(bs)[1] for _ in range(i % 4)]
                default = struct.unpack('>l', bytes([next(bs)[1] for _ in range(4)]))[0]
                ...
                out.append([i, opc['name'], padd + [default, npairs] + match_offsets])
            elif b == 196:
                op1 = next(bs)
                if op1 == 84:
